...
$aColumns = [
    ...
    db_prefix().'clients.doa as datecreated',
];
...
$areaStatusIds = [];
foreach ($this->ci->invoices_model->get_area() as $status) {
    if ($this->ci->input->post('area_' . $status['zip'])) {
        array_push($areaStatusIds, $status['zip']);
    }
}
if (count($areaStatusIds) > 0) {
    $areaStatusIds = str_replace(' ','',$areaStatusIds);
    array_push($filter, 'AND '.db_prefix().'clients.zip IN  ("' . implode(',', $areaStatusIds) . '")');
}


$fromdateIds = [];
foreach ($this->ci->invoices_model->get_monthsofcustomers() as $status) {
    if ($this->ci->input->post('fromdate_' . $status['months'])) {
        array_push($fromdateIds, $status['months']);
    }
}
if (count($fromdateIds) > 0) {
    $fromdateIds = str_replace(' ','',$fromdateIds);
    array_push($filter, 'AND MONTHNAME('.db_prefix().'clients.datecreated) IN ("' . implode(',', $fromdateIds) . '")');
}



$salepersonsIds = [];
foreach ($this->ci->invoices_model->get_salepersons() as $months) {
    if ($this->ci->input->post('salepersons_' . str_replace(' ','-',$months['months']))) {
        array_push($salepersonsIds, str_replace('-',' ',$months['months']));
    }
}
if (count($salepersonsIds) > 0) {
    $salepersonsIds = str_replace(' ','',$salepersonsIds);
    array_push($filter, 'AND '.db_prefix().'clients.salepersons IN ("' . implode(',', $salepersonsIds) . '")');
}



$sourcesIds = [];
foreach ($this->ci->invoices_model->get_sources() as $months) {
    if ($this->ci->input->post('sources_' . str_replace(' ','-',$months['months']))) {
        array_push($sourcesIds, str_replace('-',' ',$months['months']));
    }
}
if (count($sourcesIds) > 0) {
    $sourcesIds = str_replace(' ','',$sourcesIds);
    array_push($filter, 'AND '.db_prefix().'clients.sources IN ("' . implode(',', $sourcesIds) . '")');
}



$requirementss = [];
foreach ($this->ci->invoices_model->get_requirements() as $months) {
    if ($this->ci->input->post('requirements_' . str_replace(' ','-',$months['months']))) {
        array_push($requirementss, str_replace('-',' ',$months['months']));
    }
}

if (count($requirementss) > 0) {
    $requirementss = str_replace(' ','',$requirementss);
    array_push($filter, 'AND '.db_prefix().'clients.requirements IN ("' . implode(',', $requirementss) . '")');
}

if($this->ci->input->post('fromdates',TRUE) != ""){
array_push($filter, 'AND '.db_prefix().'clients.doa >=  "'.$this->ci->input->post('fromdates',TRUE).'"');
}

if($this->ci->input->post('todates',TRUE) != ""){
array_push($filter, 'AND '.db_prefix().'clients.doa <=  "'.$this->ci->input->post('todates',TRUE).'"');
}

$product_categorys = [];
foreach ($this->ci->invoices_model->get_product_category() as $months) {
    if ($this->ci->input->post('product_category_' . str_replace(' ','-',$months['months']))) {
        array_push($product_categorys, str_replace('-',' ',$months['months']));
    }
}
if (count($product_categorys) > 0) {
    $product_categorys = str_replace(' ','',$product_categorys);
    array_push($filter, 'AND '.db_prefix().'clients.product_category IN ("' . implode(',', $product_categorys) . '")');
}
