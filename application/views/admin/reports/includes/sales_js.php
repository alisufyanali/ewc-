...
<script>
 ...
 var report_items_sumary = $('#items-report-summary');
 ...
 $(function() {
   ...
   $('.table-items-report-summary').on('draw.dt', function() {
     var itemsTable = $(this).DataTable();
     var sums = itemsTable.ajax.json().sums;
     $(this).find('tfoot').addClass('bold');
     $(this).find('tfoot td').eq(0).html(")");
     $(this).find('tfoot td.amounts').html(sums.total_meter);
     $(this).find('tfoot td.qtys').html(sums.total_feet);
     $(this).find('tfoot td.ranking').html(sums.ranking);
     $(this).find('tfoot td.rate').html(sums.rate);
     $(this).find('tfoot td.discount').html(sums.discount);
     $(this).find('tfoot td.purchaseprice').html(sums.purchaseprice);
     $(this).find('tfoot td.grossmargin').html(sums.grossmargin);
     $(this).find('tfoot td.percentagemargin').html(sums.percentagemargin);
 ...
 function init_report(e, type) {
   ...
   report_items_sumary.addClass('hide');
   ...
   // Clear custom date picker
       ...
       if (type == 'total-income') {
        ...
        report_proposals.removeClass('hide');
          ...
          report_items_sumary.removeClass('hide');
      }
      else if(type == 'items-report-summary'){
          report_items_sumary.removeClass('hide');
      }
      gen_reports();
    }
   ...
   function customers_supplier() {
     if ($.fn.DataTable.isDataTable('.table-customers-supplier')) {
       $('.table-customers-supplier').DataTable().destroy();
     }
     initDataTable('.table-customers-supplier', admin_url + 'reports/customers_report', false, false, fnServerParams, [0, 'asc']);
   }
 ...
 function items_report_summary(){
   if ($.fn.DataTable.isDataTable('.table-items-report-summary')) {
     $('.table-items-report-summary').DataTable().destroy();
   }
   initDataTable('.table-items-report-summary', admin_url + 'reports/itemssummaryvendorwise', false, false, fnServerParams, [0, 'asc']);
 }
   ...
   function gen_reports() {
     ...
     if (!$('.chart-income').hasClass('hide')) {
      ...
      items_report();
      items_report_summary();
    } else if(!report_items_sumary.hasClass('hide')) {
      items_report_summary();
