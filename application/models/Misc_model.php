...
class Misc_model extends App_Model
{
    ...
    public function getGatepassByID($gatepassid){
         $this->db->select('*');
         $this->db->where('gatepassid', $gatepassid);
         return $this->db->get(db_prefix() . 'gatepass')->result_array();
    }
    
    public function getCarpetSevicesByID($gatepassid){
         $this->db->select('*');
         $this->db->where('carpetid', $gatepassid);
         return $this->db->get(db_prefix() . 'carpetservices')->result_array();
    }
    
    public function getApprovedProductById($gatepassid){
        $this->db->select('*');
        $this->db->where('uniqueid', $gatepassid);
        return $this->db->get(db_prefix() . 'approved_products')->result_array();
    }
    
    
    public function getallproducts(){
         $this->db->select('tblitems.*,tblwh_sub_group.*,tblitems_groups.name as categoryname');
         $this->db->join('tblwh_sub_group','tblwh_sub_group.id = tblitems.sub_group');
         $this->db->join('tblitems_groups','tblitems_groups.id = tblitems.group_id');
         return $this->db->get(db_prefix() . 'items')->result_array();
    }
    
    public function getallsubgroup(){
         $this->db->select('tblwh_sub_group.*');
    ...
    public function getallApprovedProducts($skucode = '',$code  = ''){
        $this->db->select('*');
        
        if($skucode != ""){
            $this->db->where('skucode',$skucode);
        }
    ...
    public function getallcarpetservices($fromclientdate = '',$toclientdate = '',$fromhandoverdate = '',$tohandoverdate = ''){
        
        if($fromclientdate != ""){
            $this->db->where('date > ',$fromclientdate);
        }
        
        if($toclientdate != ""){
            $this->db->where('date < ',$toclientdate);
        }
        
        if($fromhandoverdate != ""){
            $this->db->where('handovertovendor > ',$fromhandoverdate);
        }
        
        if($tohandoverdate != ""){
            $this->db->where('handovertovendor < ',$tohandoverdate);
        }
    ...
    public function getallrouting(){
        return $this->db->get(db_prefix() . 'routing')->result_array();
    }
    
    public function getallworkcenter(){
        return $this->db->get(db_prefix() . '_workcenter')->result_array();
    }
