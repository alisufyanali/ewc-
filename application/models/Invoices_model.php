...
class Invoices_model extends App_Model
{
    ...
    public function get_area(){
         return $this->db->query('SELECT zip FROM `tblclients` GROUP BY zip')->result_array();
    }
    
    public function get_monthsofcustomers(){
        return $this->db->query('SELECT MONTHNAME(datecreated) as months FROM `tblclients` GROUP BY months')->result_array();
    }
    
    public function get_salepersons(){
        return $this->db->query('SELECT salepersons as months FROM `tblclients` GROUP BY salepersons')->result_array();
    }
    
    public function get_sources(){
        return $this->db->query('SELECT sources as months FROM `tblclients` GROUP BY sources')->result_array();
    }
    
    public function get_requirements(){
        return $this->db->query('SELECT requirements as months FROM `tblclients` GROUP BY requirements')->result_array();
    }
    
    public function get_product_category(){
        return $this->db->query('SELECT product_category as months FROM `tblclients` GROUP BY product_category')->result_array();
    }
    ...
    public function get_purchase_reports()
    {
	   ...
	   $this->db->order_by('id,YEAR(date_add)', 'desc');
	   $invoices = $this->db->get()->result();
	   return $invoices;
    }
    
    
    public function get_allitems_reports(){
        $this->db->select('*');
    ...
    public function add($data, $expense = false)
    {
        ...
        if ($insert_id) {
            ...
            foreach ($items as $key => $item) {
                if ($itemid = add_new_sales_item_post($item, $insert_id, $meritype)) {
    ...
    public function set_invoice_paid($id, $manually = false, $emails_sent = [], $is_status_updated = false)
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'invoices', [
            'status'     => 2
