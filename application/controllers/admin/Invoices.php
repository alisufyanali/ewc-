...
class Invoices extends AdminController
{
    ...
    public function nongstinvoices($id = '')
    {
        $this->list_invoicesnongst($id);
    }
    ...
    public function list_invoices($id = '')
    {
     ...
     public function list_invoicesnongst($id = '')
    {
        if (!has_permission('invoices', '', 'view')
            && !has_permission('invoices', '', 'view_own')
            && get_option('allow_staff_view_invoices_assigned') == '0') {
        ...
        $data['invoices_statuses']    = $this->invoices_model->get_statuses();
        $data['bodyclass']            = 'invoices-total-manual';
        $this->load->view('admin/invoices/managenongst', $data);
    }
    ...
    public function tablenongst($clientid = '')
    {
        if (!has_permission('invoices', '', 'view')
            && !has_permission('invoices', '', 'view_own')
            && get_option('allow_staff_view_invoices_assigned') == '0') {
        ...
        $this->app->get_table_data(($this->input->get('recurring') ? 'recurring_invoices' : 'invoicesnongst'), [
            'clientid' => $clientid,
            'data'     => $data,
        ]);
    }
    ...
    public function waranty($id = '')
    {
        ...
        $data['title']     = $title;
        $data['bodyclass'] = 'invoice';
        $this->load->view('admin/invoices/waranty', $data);
    }
    ...
    public function mark_as_paid($id)
    {
        ...
        $success = $this->invoices_model->set_invoice_paid($id, true);

        if ($success) {
            set_alert('success', _l('invoice_marked_as_paid'));
        } else {
            set_alert('warning', _l('invoice_marked_as_paid_failed'));
        }
