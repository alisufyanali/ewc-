...
class Reports extends AdminController
{
    ...
    public function purchase_categorywise(){
        $data['title'] = 'Purchase Report Category Wise';
        $this->load->view('admin/reports/purchasereport', $data);
    }
    
    public function purchase_detailwise(){
        
        $this->load->model('invoices_model');
        $data['purchase_reports'] =  $this->invoices_model->get_purchase_reports();
        $data['title'] = 'Purchase Report Detailed';
        $this->load->view('admin/reports/purchasereportdetailed', $data);
        
    }
    
    public function stock_take_worksheet(){
        
        $this->load->model('invoices_model');
        $data['items'] =  $this->invoices_model->get_allitems_reports();
        $data['title'] = 'Stock Take Worksheet';
        $this->load->view('admin/reports/stocktakeworksheets', $data);
    ...
    public function items()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $aColumns = [
                    ...
                    db_prefix() . 'itemable.sku_code as sku_code',
                    db_prefix() . 'itemable.feet as feet',
                    db_prefix() . 'itemable.meter as meter',
                    '((' . db_prefix() . 'itemable.qty)) as quantity_sold',
                    'rate as rate',
                    '(rate*qty) as avg_price',
                    db_prefix() . 'itemable.discount'
            ];
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] = $aRow['discount'];
                $row[] = $aRow['avg_price'];
                $row[] = getPurchasePrice($aRow['sku_code']);
                $row[] = $aRow['avg_price'] - getPurchasePrice($aRow['sku_code']);
                $row[] = ($aRow['avg_price'] - getPurchasePrice($aRow['sku_code'])) / $aRow['avg_price'] * 100;
                $row[] = getVendor($aRow['sku_code']);
                $row[] = $aRow['quantity_sold'] * getPurchasePrice($aRow['sku_code']);
    ...
    public function itemssummaryvendorwise()
    {
        if ($this->input->is_ajax_request()) {
            $this->load->model('currencies_model');
            $v = $this->db->query('SELECT VERSION() as version')->row();
            ...
            $aColumns = [
                    'SUM(tblitemable.meter) as tmtr',
                    'SUM(tblitemable.feet) as tft',
                    db_prefix() . 'itemable.description as description',
                    db_prefix() . 'itemable.long_description as long_description',
                    db_prefix() . 'itemable.sku_code as sku_code',
                    db_prefix() . 'itemable.feet as feet',
                    db_prefix() . 'itemable.meter as meter',
                    '((' . db_prefix() . 'itemable.qty)) as quantity_sold',
                    db_prefix() . 'itemable.rate as rate',
                    '('.db_prefix() . 'itemable.rate * '.db_prefix() . 'itemable.qty) as avg_price',
                    db_prefix() . 'itemable.discount'
            ];
            
            $sIndexColumn = 'id';
            $sTable       = db_prefix() . 'itemable';
            $join         = ['JOIN ' . db_prefix() . 'invoices ON ' . db_prefix() . 'invoices.id = ' . db_prefix() . 'itemable.rel_id JOIN tblitems ON tblitems.sku_code = tblitemable.sku_code'];
            ...
            array_push($where, 'GROUP BY commodity_type');
            ...
            foreach ($rResult as $aRow) {
                ...
                $footer_data['total_meter'] += $aRow['tmtr'];
                $footer_data['total_feet'] += $aRow['tft'];
                $footer_data['ranking'] += $ranking;
                
                $footer_data['rate'] += $aRow['rate'];
                $footer_data['discount'] += $aRow['discount'];
                $footer_data['purchaseprice'] +=  getPurchasePrice($aRow['sku_code']);
                $footer_data['grossmargin'] += getPurchasePrice($aRow['sku_code']);
                $footer_data['percentagemargin'] += ($aRow['avg_price'] - getPurchasePrice($aRow['sku_code'])) / $aRow['avg_price'] * 100;
