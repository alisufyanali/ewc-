...
function smi_custom_view(e, t, a, i) {
    var n = void 0 === a ? "custom_view" : a;
    var e = $('#' + n).val();
    do_filter_active(n) != n && (e = ""), $('input[name="' + n + '"]').val(e), $(t).DataTable().ajax.reload();
}
    ...
    if (d <= r) {
        ...
        0 != c.length && "" !== c.val() && ((o.sort_by = c.val()), (o.sort = p)),
            ...
            requestGet(buildUrl(admin_url + e, o))
                .done(function (e) {
                    d++,
                        $('[data-load-status="' + t + '"]').before(e),
                        $('[data-col-status-id="' + t + '"]').attr("data-total", parseInt($('[data-col-status-id="' + t + '"]').attr("data-total")) + $("<ul></ul>").append(e).find("li").length),
                        $(a).attr("data-page", d),
...
function leads_kanban_update(t, e) {
    if (e === t.item.parent()[0]) {
        var i = { status: $(t.item.parent()[0]).attr("data-lead-status-id"), leadid: $(t.item).attr("data-lead-id"), order: [] };
...
function tasks_kanban_update(t, e) {
    if (e === t.item.parent()[0]) {
        ...
        $.each($(t.item.parent()[0]).find("[data-task-id]"), function (e, t) {
            ...
            task_mark_as(a, $(t.item).attr("data-task-id")),
...
function init_invoicenontgst(e) {
    load_small_table_item(e, "#invoice", "invoiceid", "invoices/get_invoice_data_ajax", ".table-invoicesnongst");
}
...
function add_item_to_preview(e) {
    requestGetJSON("invoice_items/get_item_by_id/" + e).done(function (e) {
        clear_item_preview_values(),
            $('.main textarea[name="description"]').val(   e.group_name + ' ' + e.Breadth + ' x ' + e.Width),
            $('.main input[name="sku_code"]').val(e.sku_code.replace(/(<|&lt;)br\s*\/*(>|&gt;)/g, " ")),

            $('.main input[name="feet"]').val(e.Feet),
            $('.main input[name="meter"]').val(e.Meters),
        ...
        return (
            get_taxes_dropdown_template(t, n.taxname).done(function (e) {
                (p += '<input type="hidden" class="order" name="newitems[' + _ + '][order]">'),
                    ...
                    (p += '<td class="bold feet"><input name="newitems[' + _ + '][feet]" class="form-control" rows="5" value="' + n.feet + '" style="width: 97px;"></td>'),
                    (p += '<td class="bold meter"><input name="newitems[' + _ + '][meter]" class="form-control" rows="5" value="' + n.meter + '" style="width: 97px;"></td>'),
                    (p += '<td class="bold sku_code"><textarea name="newitems[' + _ + '][sku_code]" class="form-control" rows="5">' + n.sku_code + "</textarea></td>"),
                    (p += '<td><textarea name="newitems[' + _ + '][long_description]" class="form-control item_long_description" rows="5">' + n.long_description.replace(d, "\n") + "</textarea></td>");
                ...
                0 < t.length &&
                    ...
                    (p +=
                        ...
                        '" onblur="calculate_total();" onchange="calculate_total();" name="newitems[' + _ + '][rate]" value="' + n.rate + '" data-value="' + n.rate + '" class="form-control rates"></td>'),
                    (p += '<td class="discs"><input type="number" min="0" onblur="calculate_total();" onchange="calculate_total();" data-quantity name="newitems[' + _ + '][discount]" value="' + n.discount + '" class="form-control">'),
...
function get_item_preview_values() {
    ...
    return (
        ...
        (e.sku_code = $('.main input[name="sku_code"]').val()),
        (e.long_description = $('.main textarea[name="long_description"]').val()),
        (e.feet = $('.main input[name="feet"]').val()),
        (e.meter = $('.main input[name="meter"]').val()),
        ...
        (e.discount = $('.main input[name="discount"]').val()),
        e
    );
}
...
function calculate_total_old() {
    ...
    var a,
        ...
        oc = $('input[name="othercharges"]').val(),
        fc = $('input[name="freightcharges"]').val(),
    ...
    $(".tax-area").remove(),
        $.each(p, function () {
            "" === (t = $(this).find("[data-quantity]").val()) && ((t = 1), $(this).find("[data-quantity]").val(1)),
                (s = accounting.toFixed(($(this).find("td.rate input").val() - $(this).find("td.discs input").val()) * t, app.options.decimal_places)),
                (s = parseFloat(s)),
...
//     var otherCharges = parseFloat($('input[name="othercharges"]').val()) || 0;
//     var freightCharges = parseFloat($('input[name="freightcharges"]').val()) || 0;
//     var discountPercent = parseFloat($('input[name="discount_percent"]').val()) || 0;
//     var discountTotal = parseFloat($('input[name="discount_total"]').val()) || 0;
...
//         var quantity = parseFloat($(this).find("[data-quantity]").val()) || 1;
//         var rate = parseFloat($(this).find("td.rate input").val()) || 0;
//         var discounts = parseFloat($(this).find("td.discs input").val()) || 0;
//         // var itemSubtotal = (rate - discounts) * quantity;
...
//     if (discountType && (discountPercent !== 0 || discountTotal !== 0)) {
//         if (discountType === "before_tax" && discountPercent !== 0) {
//             discountAmount += (subtotal * discountPercent) / 100;
//             discountAmount = Math.abs(discountAmount)
//         } else if (discountType === "before_tax" && discountTotal !== 0) {
...
//     // Calculate total after taxes and adjustments
//     var total = subtotal + adjustment + otherCharges + freightCharges;

//     console.log("Total after adjustments:", total);

//     if(discountAmount !== 0){
//         total -= discountAmount;
//     }

//     // Adjust total based on discounts
//     // if (discountType && (discountPercent !== 0 || discountTotal !== 0)) {
//     //     total -= discountAmount;
//     // }

//     // Update discount total input field
//     // $('input[name="discount_total"]').val(accounting.toFixed(discountAmount, app.options.decimal_places));

//     // Display formatted discount amount
//     $(".discount-total").html( format_money(discountAmount));

//     // Display values for subtotal, adjustments, and total
...
//     // Trigger event indicating sales total calculation completion
//     $(document).trigger("sales-total-calculated");
// }
...
function calculate_total() {
    ...
    var taxData = {}; // To store tax rates and amounts
    ...
    $(".table.has-calculations tbody tr.item").each(function () {
        ...
        var rate = parseFloat($(this).find("td.rate input").val()) || 0;
        var discounts = parseFloat($(this).find("td.discs input").val()) || 0;
        var itemSubtotal = (rate) * quantity; // Updated calculation for subtotal
        
        subtotal += itemSubtotal;
        discountAmount += discounts;

        // Calculate tax
        var taxRates = $(this).find("select.tax").selectpicker("val");
        if (taxRates) {
            $.each(taxRates, function (index, tax) {
                var taxRateParts = tax.split("|");
                var taxRate = taxRateParts[1];
                var taxAmount = (itemSubtotal / 100) * taxRate;
                if (taxData.hasOwnProperty(tax)) {
                    taxData[tax] += taxAmount;
                } else {
                    taxData[tax] = taxAmount;
                }
    ...
    // Adjust total based on discounts
    if (discountType && (discountPercent !== 0 || discountTotal !== 0) && discountType === "after_tax") {
        if (discountPercent !== 0) {
    ...
    // Update the DOM with calculated taxes
    $("#discount_area").siblings(".tax-area").remove(); // Remove old tax rows
    $.each(taxData, function (tax, amount) {
        var taxRate = tax.split("|");
        var taxRow = '<tr class="tax-area"><td>' + taxRate[0] + '(' + taxRate[1] + '%)</td><td>' + format_money(amount) + '</td></tr>';
        $("#discount_area").after(taxRow);
    });
...
function estimates_pipeline_update(t, e) {
    if (e === t.item.parent()[0]) {
        var i = { estimateid: $(t.item).attr("data-estimate-id"), status: $(t.item.parent()[0]).attr("data-status-id"), order: [] };
...
function proposals_pipeline_update(t, e) {
    if (e === t.item.parent()[0]) {
        var i = { order: [], status: $(t.item.parent()[0]).attr("data-status-id"), proposalid: $(t.item).attr("data-proposal-id") };
        $.each($(t.item).parents(".pipeline-status").find("li"), function (e, t) {
...
$("body").on("loaded.bs.select change", "select.ajax-search", function (e) {
    ...
    $(function () {
        ...
        if (((table_invoicesnongst = $("table.table-invoicesnongst")), (table_estimates = $("table.table-estimates")), 0 < table_invoicesnongst.length || 0 < table_estimates.length)) {
            var f = {},
                h = $("._hidden_inputs._filters input");
            $.each(h, function () {
                f[$(this).attr("name")] = '[name="' + $(this).attr("name") + '"]';
            }),
                table_invoicesnongst.length &&
                initDataTable(
                    table_invoicesnongst,
                    admin_url + "invoices/tablenongst" + ($("body").hasClass("recurring") ? "?recurring=1" : ""),
                    "undefined",
                    "undefined",
                    f,
                    $("body").hasClass("recurring")
                        ? [table_invoicesnongst.find("th.next-recurring-date").index(), "asc"]
                        : [
                            [3, "desc"],
                            [0, "desc"],
                        ]
        ...
        $("body").on("submit", "._transaction_form", function () {
            ...
            $("body").on("change", 'input[name="discount_percent"],input[name="discount_total"]', function () {
                // Check if discount type is not selected and current input has a value
                if ("" === $('select[name="discount_type"]').val() && $(this).val() != 0) {
                    // Check if there are any elements with IDs starting with 'tax_id_'
                    if ($("td[id^='tax_id_']").length === 0) {
                            ...
                            // Highlight the label associated with 'discount_type' input
                        ...
                        return; // Exit function if no tax elements found
                    }
