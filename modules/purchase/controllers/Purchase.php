...
class purchase extends AdminController
{
     ...
     */
     ...
     public function vendorimport(){
        $this->load->view('vendors/vendorimport'); 
     }
     
     public function importTst()
    {
        ...
        if (($handle = fopen($_FILES['userfile']['tmp_name'], 'r')) !== FALSE)
        {
         ...
         while($csv_line = fgetcsv($fp,1024)){
                ...
                {                  
                  //                
                  $insert_csv = array();
                  $insert_csv['company']    = (!empty($csv_line[0])?$csv_line[0]:null);
                  $insert_csv['vat']   = (!empty($csv_line[1])?$csv_line[1]:null);
                  $insert_csv['phonenumber']  = (!empty($csv_line[2])?$csv_line[2]:null);
                  $insert_csv['country']    = (!empty($csv_line[3])?$csv_line[3]:null);
                  $insert_csv['city']          = (!empty($csv_line[4])?$csv_line[4]:null);
                  $insert_csv['zip'] = (!empty($csv_line[5])?$csv_line[5]:null);
                  $insert_csv['state'] = (!empty($csv_line[6])?$csv_line[6]:null);
                  $insert_csv['address'] = (!empty($csv_line[7])?$csv_line[7]:null);
                  $insert_csv['website'] = (!empty($csv_line[8])?$csv_line[8]:null);
                  $insert_csv['billing_street'] = (!empty($csv_line[9])?$csv_line[11]:null);
                  $insert_csv['billing_city'] = (!empty($csv_line[10])?$csv_line[12]:null);
                  $insert_csv['billing_state'] = (!empty($csv_line[11])?$csv_line[11]:null);
                  $insert_csv['billing_zip'] = (!empty($csv_line[12])?$csv_line[12]:null);
                  $insert_csv['billing_country'] = (!empty($csv_line[13])?$csv_line[13]:null);
                  $insert_csv['shipping_street'] = (!empty($csv_line[14])?$csv_line[14]:null);
                  $insert_csv['shipping_city'] = (!empty($csv_line[15])?$csv_line[15]:null);
                  $insert_csv['shipping_state'] = (!empty($csv_line[16])?$csv_line[16]:null);
                  $insert_csv['shipping_zip'] = (!empty($csv_line[17])?$csv_line[17]:null);
                  $insert_csv['shipping_country'] = (!empty($csv_line[18])?$csv_line[18]:null);
                  $insert_csv['longitude'] = (!empty($csv_line[19])?$csv_line[19]:null);
                  $insert_csv['latitude'] = (!empty($csv_line[20])?$csv_line[20]:null);
                  $insert_csv['stripe_id'] = (!empty($csv_line[21])?$csv_line[21]:null);
                }
                 
                     $supplierinfo=array(             
            'company' => $insert_csv['company'],
            'vat'           => $insert_csv['vat'],
            'phonenumber'            => $insert_csv['phonenumber'],
            ...
            'billing_street'            => $insert_csv['billing_street'],
            'billing_city'            => $insert_csv['billing_city'],
            'billing_state'            => $insert_csv['billing_state'],
            'billing_zip'            => $insert_csv['billing_zip'],
            'billing_country'            => $insert_csv['billing_country'],
            'shipping_street'            => $insert_csv['shipping_street'],
            'shipping_city'            => $insert_csv['shipping_city'],
            'shipping_state'            => $insert_csv['shipping_state'],
            'shipping_zip'            => $insert_csv['shipping_zip'],
            'shipping_country'            => $insert_csv['shipping_country'],
            'longitude'            => $insert_csv['longitude'],
            'latitude'            => $insert_csv['latitude'],
            'stripe_id'            => $insert_csv['stripe_id']
            );
                     if ($count > 0) {
            $this->db->insert('tblpur_vendor',$supplierinfo);
        }
        ...
        redirect(base_url('admin/purchase/vendorimport'));
        }
            else{
    ...
    public function add_payment($pur_order){
         if ($this->input->post()) {
            ...
            set_alert('success', $message);
            redirect(admin_url('warehouse/manage_purchase/'.$pur_order));
