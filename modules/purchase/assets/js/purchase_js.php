...
function wh_add_item_to_preview(id) {
	...
	requestGetJSON('warehouse/get_item_by_id/' + id).done(function (response) {
		...
		$('.main textarea[name="commodity_name"]').val(response.group_name + ' ' + response.Breadth + ' X ' + response.Width);
...
function wh_get_item_row_template(name, commodity_name, warehouse_id, quantities, unit_name, unit_price, taxname, lot_number, date_manufacture, expiry_date, commodity_code, unit_id, tax_rate, tax_money, goods_money, note, item_key,feet,Meter,SKU,Description)  {
	"use strict";
...
function wh_calculate_total(){
	...
	$('.wh-subtotal').html(currency + ' ' + ss);
	

	$('.inventory_value').remove();
	var total_inventory_value = '<tr class="inventory_value"><td><span class="bold"><?php echo _l('value_of_inventory'); ?> :</span></td><td class="">' + currency + ' ' +subtotal+'</td></tr>';
	$('#subtotal').after(total_inventory_value);

	$('.total_tax_value').remove();
	var total_tax_value = '<tr class="total_tax_value"><td><span class="bold"><?php echo _l('total_tax_money'); ?> :</span></td><td class="">' + currency + ' ' +total_tax_money+'</td></tr>';
	...
	$('.wh-total').html(currency + ' ' + total);
	var rconversion = $('input[name="rconversion"]').val();
    $('.wh-subtotals').html(currency + ' ' + (parseInt(total) * parseInt(rconversion)));
