...
$subgroup_ft  = $this->ci->input->post('subgroup_ft');

$group_ft =  $this->ci->input->post('group_ft');
$qty_filter = $this->ci->input->post('qty_ft');

$commodity_ft = $this->ci->input->post('commodity_ft');
$alert_filter = $this->ci->input->post('alert_filter');
$group_filter = $this->ci->input->post('group_ft');
$qty_filter = $this->ci->input->post('qty_ft');
...
if (isset($warehouse_ft)) {
    ...
    foreach ($warehouse_ft as $commodity_id) {
	$where[] = 'AND '.db_prefix().'items.commodity_type = "'.$commodity_id.'"';
    }
}

if (isset($subgroup_ft)) {
    foreach ($subgroup_ft as $commodity_id) {
	$where[] = 'AND '.db_prefix().'items.sub_group = "'.$commodity_id.'"';
    }
}
	...
	foreach ($commodity_ft as $commodity_id) {
		if ($commodity_id != '') {
			if ($where_commodity_ft == '') {
				$where_commodity_ft .= 'AND (' . db_prefix() . 'items_groups.name = "' . $commodity_id . '"';
			} else {
				$where_commodity_ft .= ' or ' . db_prefix() . 'items_groups.name = "' . $commodity_id . '"';
			}
		}
	}
	if ($where_commodity_ft != '') {
...
$item_have_variation = $this->ci->warehouse_model->arr_item_have_variation();
	...
	foreach ($rResult as $aRow) {
		...
		if($aRow['holdreason'] != ""){
		    $row['12'] = '<span style="font-weight: 900;color: #fd6e0d;font-size: 13px;"> HOLD , REASON : '.$aRow['holdreason'].'</span>';
		} 
		else{
		    $row['12'] = number_format((getTotalPurchaseQuantity($aRow['id']) - getTotalReturnQuantity($aRow['id'])) - (getTotalInvoiceQuantity(str_replace(' ', '' ,$aRow['sku_code'])) - getTotalRefundQuantity(str_replace(' ', '' ,$aRow['sku_code']))));
		}
...
if (isset($qty_filter)) {
    ...
    else if($qty_filter == 0){
        if($row['12'] <= 0){
            $output['aaData'][] = $row;
        }
    }
    else if($qty_filter == 1){
        if($row['12'] > 0){
            $output['aaData'][] = $row;
        }
    }
