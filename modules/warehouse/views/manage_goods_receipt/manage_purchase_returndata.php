...
if (isset($day_vouchers)) {
    ...
    $where[] = 'AND tblgoods_return.date_add <= "' . $day_vouchers . '"';
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if($aColumns[$i] == 'supplier_name'){
            ...
            $name = '<a href="' . admin_url('warehouse/view_purchasereturn/' . $aRow['id'] ).'" onclick="init_goods_return('.$aRow['id'].'); return false;">' . str_replace('PUR','PURRETURN',$aRow['goods_receipt_code']) . '</a>';

            $name .= '<div class="row-options">';

            $name .= '<a href="' . admin_url('warehouse/edit_return/' . $aRow['id'] ).'" >' . _l('view') . '</a>';
            $name .= ' | <a href="' . admin_url('warehouse/manage_goods_return/' . $aRow['id'] ).'" >Edit</a>';
