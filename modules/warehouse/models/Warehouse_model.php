...
class Warehouse_model extends App_Model {
    ...
    public function get_purchase_vendors() {
		return $this->db->query('select * from tblpur_vendor')->result_array();
	}
	...
	public function get_purchase_vendor() {
	...
	public function add_goods_receipt($data, $id = false) {
		...
		$data['subtotals'] = reformat_currency_j($data['subtotals']);
	...
	public function add_goods_return($data, $id = false) {
		...
		$data['goods_receipt_code'] = str_replace('PUR','PR-',$this->create_goods_code());
		...
		$this->db->insert(db_prefix() . 'goods_return', $data);
		$insert_id = $this->db->insert_id();
		...
		if ($insert_id) {
			foreach ($inventory_receipts as $inventory_receipt) {
				...
				$this->db->insert(db_prefix() . 'goods_return_detail', $inventory_receipt);
				if($this->db->insert_id()){
					$results++;
				}
			}
    ...
    public function get_goods_returns($id) {
		if (is_numeric($id)) {
			$this->db->where('id', $id);

			return $this->db->get(db_prefix() . 'goods_return')->row();
		}
		if ($id == false) {
			return $this->db->query('select * from tblgoods_return')->result_array();
		}
	}
	...
	public function get_goods_returns_detail($id) {
		if (is_numeric($id)) {
			$this->db->where('goods_receipt_id', $id);

			return $this->db->get(db_prefix() . 'goods_return_detail')->result_array();
		}
		if ($id == false) {
			return $this->db->query('select * from tblgoods_return_detail')->result_array();
		}
	}
	...
	public function get_pur_request($pur_order) {
		...
		$sql = 'select item_code as commodity_code, ' . db_prefix() . 'items.description, ' . db_prefix() . 'items.unit_id, unit_price, quantity as quantities, ' . db_prefix() . 'pur_order_detail.tax as tax, into_money, (' . db_prefix() . 'pur_order_detail.total-' . db_prefix() . 'pur_order_detail.into_money) as tax_money, total as goods_money,  '.db_prefix().'pur_order_detail.id as id from ' . db_prefix() . 'pur_order_detail
	...
	public function get_stock_import_pdf_html($goods_receipt_id) {
		...
		<tbody>
		<tr>
		    <td class="text-left">'.$organization_info.'</td>
		</tr>
		<tr>
		    <td class="text-left">'.$bill_to.'</td>
		</tr>
		<tr>
		<td class="text_right">'.$invoice_date.'</td>
		</tr>
		</tbody>
		</table>
		';
		...
		<td ></td>
		<td ></td>
		<td class="text_left"><b>' . _l('total') . '</b></td>
		<td class="text_right">$' . app_format_money((float)($goods_receipt->total_goods_money+$goods_receipt->discount), '') . '</td>
		</tr><tr>
		<td ></td>
		<td ></td>
		<td ></td>
		<td class="text_left"><b>' . _l('discount') . '</b></td>
		<td class="text_right">$' . app_format_money((float) $goods_receipt->discount, '') . '</td>
		</tr><tr>
		<td ></td>
		<td ></td>
		<td ></td>
		<td class="text_left"><b>' . _l('total_goods_money') . '</b></td>
		<td class="text_right">$' .app_format_money((float) $goods_receipt->total_goods_money, '') . '</td>
		</tr>
		...
		<td class="text_right">$' . app_format_money((float) $goods_receipt->value_of_inventory, '') . '</td>
		</tr>';
		...
		<td class="text_right">$' . app_format_money((float) $goods_receipt->total_tax_money, '') . '</td>
		</tr>';
		...
		<td class="text_right">$' . app_format_money((float) $goods_receipt->total_money, '') . '</td>
		</tr>
	...
	public function get_vendor_ajax($pur_orders_id) {
		$data = [];
		$sql = 'SELECT *, ' . db_prefix() . 'pur_request.requester FROM ' . db_prefix() . 'pur_vendor
		left join ' . db_prefix() . 'pur_orders on ' . db_prefix() . 'pur_vendor.userid = ' . db_prefix() . 'pur_orders.vendor
		left join ' . db_prefix() . 'pur_request on ' . db_prefix() . 'pur_orders.discount_type = ' . db_prefix() . 'pur_request.id
	...
	public function get_stock_export_pdf_html($goods_delivery_id) {
		...
		<tbody>
		<tr>
		<td rowspan="2" width="50%" class="text-left"><h3>' . mb_strtoupper(_l('delivery')) . ' #'.$goods_delivery->goods_delivery_code.'</h3>'.$organization_info.'</td>
		<td class="text_right_weight ">'.$bill_to.'</td>
		</tr>

		<tr>
		<td class="text_right">'.$ship_to.'</td>
		</tr>
		</tbody>
		</table>
		<br><br><br>
		...
		<th  class=" thead-dark">Feet</th>
		<th  class=" thead-dark">Meter</th>
		<th  class=" thead-dark">SKU</th>
		...
		foreach ($goods_delivery_detail as $delivery_key => $delivery_value) {
			...
			<td class="td_style_r_ep_c"><b>' . get_commodity_name($delivery_value['commodity_code'])->Feet . '</b></td>
			<td class="td_style_r_ep_c"><b>' . get_commodity_name($delivery_value['commodity_code'])->Meters . '</b></td>
			<td class="td_style_r_ep_c"><b>' . get_commodity_name($delivery_value['commodity_code'])->sku_code . '</b></td>
			<td class="td_style_r_ep_c"><b>' . get_commodity_name($delivery_value['commodity_code'])->long_description . '</b></td>';
	...
	public function get_stock_summary_report($data) {
        ...
        $commodity_lists = $this->db->query('SELECT ' . db_prefix() . 'items.id as commodity_id, ' . db_prefix() . 'items.commodity_code, ' . db_prefix() . 'items.rate, ' . db_prefix() . 'items.description as commodity_name FROM ' . db_prefix() . 'items where '.db_prefix().'items.active = 1 group by commodity_id')->result_array();
		...
		//import_periods
		if (strlen($where_warehouse_id) > 0) {
			$import_periods = $this->db->query('SELECT sku_code as commodity_id, qty as quantity, rate as purchase_price,rate as price FROM ' . db_prefix() . 'itemable WHERE rel_type = "Invoice"')->result_array();
	...
	public function get_stock_summary_report_view($data) {
		...
		$commodity_lists = $this->db->query('SELECT ' . db_prefix() . 'items.id as commodity_id,' . db_prefix() . 'items.long_description,' . db_prefix() . 'items.Feet,' . db_prefix() . 'items.Meters,' . db_prefix() . 'items.sub_group as sub_group,' . db_prefix() . 'items.rate as rate,' . db_prefix() . 'items.group_id as commodity_type, ' . db_prefix() . 'items.commodity_code, ' . db_prefix() . 'items.rate, ' . db_prefix() . 'items.description as commodity_name FROM ' . db_prefix() . 'items where '.db_prefix().'items.active = 1 group by commodity_id')->result_array();
    	...
    	$tp = 0;
    	$ts = 0;
    	$totalfeet = $totalmeter = $totalquantity = 0;
		foreach ($commodity_lists as $commodity_list_key => $commodity_list) {
			//get purchase price of item, before version get sales price.
			...
			if(has_permission('pricingtabs','','Purchase Price') == 1) {
			    $godod =  $purchase_price;
			    $tp = $tp +  $purchase_price;
			}
			else{
			    $godod = "NOT MENTIONED";
			}
			...
			if(has_permission('pricingtabs','','Sales Price') == 1) {
			    $godods = $commodity_list['rate'];
			    $ts = $ts +  $commodity_list['rate'];
			}
			else{
			    $godods = "NOT MENTIONED";
			}
			
			$totalfeet = $totalfeet  + $commodity_list['Feet'];
			$totalmeter = $totalmeter + $commodity_list['Meters'];
			$totalquantity = $totalquantity + (getTotalPurchaseQuantity($commodity_list['commodity_id']) - getTotalReturnQuantity($commodity_list['commodity_id'])) - (getTotalInvoiceQuantity(str_replace(' ', '' ,$commodity_list['commodity_code'])) - getTotalRefundQuantity(str_replace(' ', '' ,$commodity_list['commodity_code'])));
			
			$html .= '<tr>
			<td class="border_td">' . getItemCategory($commodity_list['commodity_type']) . '</td>
			<td class="border_td">' . getItemCategorys($commodity_list['sub_group']) . '</td>
			<td class="border_td">' . $commodity_list['long_description'] . '</td>
			<td class="border_td">' . $commodity_list['commodity_code'] . '</td>
			<td class="border_td">' . $commodity_list['commodity_name'] . '</td>
			<td class="border_td">' . $commodity_list['Feet'] . '</td>
			<td class="border_td">' . $commodity_list['Meters'] . '</td>
			<td class="bor_alir">' . number_format((getTotalPurchaseQuantity($commodity_list['commodity_id']) - getTotalReturnQuantity($commodity_list['commodity_id'])) - (getTotalInvoiceQuantity(str_replace(' ', '' ,$commodity_list['commodity_code'])) - getTotalRefundQuantity(str_replace(' ', '' ,$commodity_list['commodity_code'])))) . '</td>
			<td class="bor_alir">' . $godods . '</td>
			<td class="bor_alir">' . $godod . '</td>
		    </tr>';
		}
		$html .= '<tr>
                 <th class="text-center">CATEGORY</th>
                 ...
                 <th class="text-center">SKU CODE</th>
                 <th class="text-center">NAME</th>
                 <th class="text-center">'.$totalfeet.'</th>
                 <th class="text-center">'.$totalmeter.'</th>
	...
	public function get_inventory_valuation_report_view($data) {
		...
		if (strlen($data['warehouse_id']) > 0) {
			$commodity_lists = $this->db->query('SELECT sub_group,group_id,Feet,Meters,commodity_id, ' . db_prefix() . 'items.commodity_code, ' . db_prefix() . 'items.rate, ' . db_prefix() . 'items.purchase_price, ' . db_prefix() . 'items.description as commodity_name, ' . db_prefix() . 'ware_unit_type.unit_name FROM ' . db_prefix() . 'goods_transaction_detail
		...
		<th colspan="1" class="td_text">Category</th>
		<th colspan="1" class="td_text">Sub Category</th>
		<th colspan="1" class="td_text">MTR</th>
		<th colspan="1" class="td_text">FT</th>
		...
		foreach ($commodity_lists as $commodity_list_key => $commodity_list) {
			...
			<td class="border_1">' . getItemCategory($commodity_list['group_id']) . '</td>
			<td class="border_1">' . getItemCategorys($commodity_list['sub_group']) . '</td>
			<td class="border_1">' . $commodity_list['Meters'] . '</td>
			<td class="border_1">' . $commodity_list['Feet'] . '</td>';
	...
	public function update_goods_receipt($data, $id = false) {
		...
		$this->db->update(db_prefix() . 'goods_return', $data); //goods_receipt
		if ($this->db->affected_rows() > 0) {
			$results++;
		}
	...
	public function get_item_v2($id = '')
    {
        ...
        $this->db->select($rateCurrencyColumns . '' . db_prefix() . 'items.id as itemid,rate,' . db_prefix() . 'items.*,
            t1.taxrate as taxrate,t1.id as taxid,t1.name as taxname,
            t2.taxrate as taxrate_2,t2.id as taxid_2,t2.name as taxname_2,
            CONCAT(commodity_code,"_",description) as code_description,long_description,' . db_prefix() . 'items.group_id,' . db_prefix() . 'items_groups.name as group_name,unit,'.db_prefix().'ware_unit_type.unit_name as unit_name, purchase_price, unit_id, guarantee,sub_group_name');
     ...
     * @return [type]                    
     */
    public function create_goods_receipt_row_template($warehouse_data = [], $name = '', $commodity_name = '', $warehouse_id = '', $quantities = '', $unit_name = '', $unit_price = '', $taxname = '', $lot_number = '', $date_manufacture = '', $expiry_date = '', $commodity_code = '', $unit_id = '', $tax_rate = '', $tax_money = '', $goods_money = '', $note = '', $item_key = '',$feet = '',$Meter  = '',$SKU  = '',$Description = '',$discount = '', $sub_total = '', $tax_name = '', $tax_id = '', $is_edit = false) {
...
// 			$Description = $name . '[Description]';
		...
		$row .= '<td class="">' . render_textarea($name_commodity_name, '', $commodity_name, ['rows' => 2, 'placeholder' => _l('item_description_placeholder'), 'readonly' => true] ) .  render_input($name_note, '', $note, 'text', ['placeholder' => _l('commodity_notes')], [], 'no-margin', 'input-transparent text-left') . '<input type="hidden" name="warehouse_id" style="display: none !important" value="18"></td>';
		
		$row .= '<td><input type="text" id="feet" name="feet" class="form-control class1" value="'.$feet.'"></td>';
		$row .= '<td><input type="text" id="meter" name="meter" class="form-control" value="'.$Meter.'"></td>';
		
		$row .= '<td><input type="text" id="SKU" name="SKU" class="form-control" value="'.$SKU.'"></td>';
		$row .= '<td><input type="text" id="LongDescription" name="LongDescription" class="form-control" value="'.$Description.'"></td>';
		...
		$row .= '<td class="discount"><input type="text" id="discount" name="discount" class="form-control" value="'.$discount.'"></td>';
...
// 		$row .= '<td>' . render_date_input($name_expiry_date, '', $expiry_date, ['placeholder' => _l('expiry_date')]) . '</td>';
	...
	public function create_goods_return_row_template($warehouse_data = [], $name = '', $commodity_name = '', $warehouse_id = '', $quantities = '', $unit_name = '', $unit_price = '', $taxname = '', $lot_number = '', $date_manufacture = '', $expiry_date = '', $commodity_code = '', $unit_id = '', $tax_rate = '', $tax_money = '', $goods_money = '', $note = '', $item_key = '',$feet = '',$Meter  = '',$SKU  = '',$Description = '',$discount = '', $sub_total = '', $tax_name = '', $tax_id = '', $is_edit = false) {
		...
		$row .= '<td><input type="text" id="feet" name="feet" class="form-control class2" value="'.get_commodity_name($commodity_code)->Feet.'"></td>';
		$row .= '<td><input type="text" id="meter" name="meter" class="form-control" value="'.get_commodity_name($commodity_code)->Meters.'"></td>';
		
		$row .= '<td><input type="text" id="SKU" name="SKU" class="form-control" value="'.get_commodity_name($commodity_code)->sku_code.'"></td>';
		$row .= '<td><input type="text" id="LongDescription" name="LongDescription" class="form-control" value="'.get_commodity_name($commodity_code)->long_description.'"></td>';
...
// 		$row .= '<td>' . render_date_input($name_expiry_date, '', $expiry_date, ['placeholder' => _l('expiry_date')]) . '</td>';
    ...
    public function create_goods_delivery_row_template($warehouse_data = [], $name = '', $commodity_name = '', $warehouse_id = '', $available_quantity = '', $quantities = '', $unit_name = '', $unit_price = '', $taxname = '',  $commodity_code = '', $unit_id = '', $tax_rate = '', $total_money = '', $discount = '', $discount_money = '', $total_after_discount = '', $guarantee_period = '', $expiry_date = '', $lot_number = '', $note = '',  $sub_total = '', $tax_name = '', $tax_id = '', $item_key = '',$long_description = false ,$sku_code = false,$feet = '',$meter = '',$is_edit = false, $is_purchase_order = false) {
        ...
        $row .= '<td class="">' . render_textarea($name_commodity_name, '', $commodity_name, ['rows' => 2, 'placeholder' => 'Product', 'readonly' => true] ) . '</td>';
        $row .= '<td><input name="feet" rows="4" class="form-control class3" placeholder="Feet" style="width: 97px;" value="'.$feet.'"></td>';
        $row .= '<td><input name="meter" rows="4" class="form-control" placeholder="Meter" style="width: 97px;" value="'.$meter.'"></td>';
        $row .= '<td><input name="sku_code" rows="4" class="form-control" placeholder="SKU CODE" style="width: 155px;"  value="'.$sku_code.'"></td>';
        $row .= '<td><textarea name="long_description" rows="4" class="form-control" placeholder="'._l('item_long_description_placeholder').'" style="height: 35px;width: 211px;">'.$long_description.'</textarea></td>';
        $row .= '<td class="quantities">' . render_input($name_quantities, '', $quantities, 'number', $array_qty_attr, [], 'no-margin'). '</td>';
    ...
    public function packing_list_get_goods_delivery()
	{
		...
		if(count($arr_goods_delivery) > 0){
			return $this->db->query('select * from '.db_prefix().'goods_delivery where  id NOT IN ('.implode(",", $arr_goods_delivery).') order by id desc')->result_array();
		}
		return $this->db->query('select * from '.db_prefix().'goods_delivery order by id desc')->result_array();
	}
	...
	public function create_packing_list_row_template( $delivery_detail_id = '', $name = '', $commodity_name = '', $quantities = '', $unit_name = '', $unit_price = '', $taxname = '',  $commodity_code = '', $unit_id = '', $tax_rate = '', $total_amount = '', $discount = '', $discount_total = '', $total_after_discount = '', $sub_total = '', $tax_name = '', $tax_id = '', $item_key = '',$is_edit = false, $max_qty = false) {
		...
		$row .= '<td class="">' . render_textarea($name_commodity_name, '', $commodity_name, ['rows' => 2, 'placeholder' => _l('item_description_placeholder'), 'readonly' => true] ) . '</td>';
        $row .= '<td><input type="text" id="feet" name="feet" class="form-control class4" value="'.get_commodity_name($commodity_code)->Feet.'"></td>';
		$row .= '<td><input type="text" id="meter" name="meter" class="form-control" value="'.get_commodity_name($commodity_code)->Meters.'"></td>';
		$row .= '<td><input type="text" id="SKU" name="SKU" class="form-control" value="'.get_commodity_name($commodity_code)->sku_code.'"></td>';
		$row .= '<td><input type="text" id="LongDescription" name="LongDescription" class="form-control" value="'.get_commodity_name($commodity_code)->long_description.'"></td>';
        $row .= '<td class="quantities">'.render_input($name_quantities, '', $quantities, 'number', $array_qty_attr, [], 'no-margin').'</td>';
