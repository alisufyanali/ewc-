...
class warehouse extends AdminController {
	...
	public function check_sku_code(){
	    $CI = & get_instance();
	    $skus = $CI->input->post('skucode',TRUE);
        $user_information = $CI->db->query("SELECT * FROM `tblitems` WHERE sku_code = '".$skus."'")->row();
        echo $user_information->sku_code;
	}
	...
	public function commodity_list($id = '') {
		...
		$data['vendor'] = $this->warehouse_model->get_purchase_vendors();
		...
		$data['warehouses'] = $this->warehouse_model->get_purchase_vendor();
		...
		//filter
		$data['warehouse_filter'] = $this->warehouse_model->get_purchase_vendor();
		// $data['commodity_filter'] = $this->warehouse_model->get_commodity_active();

		$data['sub_groups'] = $this->warehouse_model->get_sub_group();
		
		$data['groups'] = $this->warehouse_model->get_group();
	...
	public function manage_purchase_returndata() {
		$this->app->get_table_data(module_views_path('warehouse', 'manage_goods_receipt/manage_purchase_returndata'));
	}
	...
	public function manage_return($id = '') {
		$data['title'] = 'Manage Purchase Return';
		$data['purchase_id'] = $id;
		$this->load->view('manage_goods_receipt/manage_return', $data);
	}
	...
	public function manage_goods_receipt($id = '') {
        ...
        $data['currencies'] = $this->warehouse_model->get_commodity();
	...
	public function edit_goods_receipt($id = '') {
		$this->load->model('clients_model');
		$this->load->model('taxes_model');
	...
	public function manage_goods_return($id = '') {
		$this->load->model('clients_model');
		$this->load->model('taxes_model');
		...
		if ($this->input->post()) {
			...
			$data = $this->input->post();
            if (!$this->input->post('id')) {
                $mess = $this->warehouse_model->add_goods_return($data);
                if ($mess) {
					if($data['save_and_send_request'] == 'true'){
						$this->save_and_send_request_send_mail(['rel_id' => $mess, 'rel_type' => '1', 'addedfrom' => get_staff_user_id()]);
					}
		...
		//check status module purchase
		if($id != ''){
			$goods_receipt = $this->warehouse_model->get_goods_returns($id);
			if (!$goods_receipt) {
				blank_page('Stock received Not Found', 'danger');
			}
			$data['goods_receipt_detail'] = $this->warehouse_model->get_goods_returns_detail($id);
			$data['goods_receipt'] = $goods_receipt;
			$data['tax_data'] = $this->warehouse_model->get_html_tax_receip($id);
			$data['total_item'] = count($data['goods_receipt_detail']);
			...
			if (count($data['goods_receipt_detail']) > 0) {
				...
				foreach ($data['goods_receipt_detail'] as $receipt_detail) {
					...
					$goods_receipt_row_template .= $this->warehouse_model->create_goods_return_row_template($warehouse_data, 'items[' . $index_receipt . ']', $commodity_name, $receipt_detail['warehouse_id'], $receipt_detail['quantities'], $unit_name, $receipt_detail['unit_price'], $taxname, $receipt_detail['lot_number'], $date_manufacture, $expiry_date, $receipt_detail['commodity_code'], $receipt_detail['unit_id'] , $receipt_detail['tax_rate'], $receipt_detail['tax_money'], $receipt_detail['goods_money'], $receipt_detail['note'], $receipt_detail['id'], $receipt_detail['feet'], $receipt_detail['Meter'], $receipt_detail['SKU'], $receipt_detail['Description'], $receipt_detail['sub_total'], $receipt_detail['tax_name'], $receipt_detail['tax'], true);
					
				}
			}

			$data['goods_receipt_detail'] = json_encode($this->warehouse_model->get_goods_returns_detail($id));

		}

		$data['goods_receipt_row_template'] = $goods_receipt_row_template;

		$this->load->view('manage_goods_receipt/purchasereturn', $data);
	...
	public function view_purchase($id) {
        ...
        $data['payment'] = $this->warehouse_model->get_payment_purchase_order($id);
	...
	public function view_purchasereturn($id) {
		...
		$data['units_warehouse_name'] = $this->warehouse_model->get_warehouse_code_name();
        $data['goods_receipt_detail'] = $this->warehouse_model->get_goods_returns_detail($id);
        $data['goods_receipt'] = $this->warehouse_model->get_goods_returns($id);
        $data['tax_data'] = $this->warehouse_model->get_html_tax_receip($id);
        $data['title'] = _l('stock_received_info');
        $data['payment'] = $this->warehouse_model->get_payment_purchase_order($id);
		$check_appr = $this->warehouse_model->get_approve_setting('1');
		...
		$base_currency = $this->currencies_model->get_base_currency();
		$data['base_currency'] = $base_currency;
        $this->load->view('manage_goods_receipt/view_return', $data);
	...
	public function edit_purchase($id) {
		...
		$this->load->model('currencies_model');
		$base_currency = $this->currencies_model->get_base_currency();
		$data['base_currency'] = getVendorcurrency($goods_receipt->supplier_code);
	...
	public function edit_return($id) {
		...
		$goods_receipt_data = $this->warehouse_model->get_goods_returns_detail($id);
		$data['goods_receipt_detail'] = json_encode($goods_receipt_data);
		$data['taxes'] = $this->warehouse_model->get_taxes();
		...
		$this->load->view('manage_goods_receipt/edit_return', $data);
...
if(strlen($data['inventory_filter']) > 0){
	...
	public function add_hold_stock()
	{
		if (!$this->input->is_ajax_request()) {
			show_404();
		}
        ...
        $data['itemid'] = $id;
		$data['title'] = 'Hold Product '.' ( '.$item_name.' )';
		$data['item_name'] =  $item_name;
       $this->load->view('item_add_hold_stock', $data);
	}
    
    public function insert_hold_stock(){
        $productid = $this->input->post('productid',TRUE);
        $holdreason = $this->input->post('holdreason',TRUE);
        
        $this->db->set('holdreason', $holdreason);
     ...
     public function insert_realease_stock($productid){
        
        $this->db->set('holdreason', '');
        $this->db->where('id', $productid);
        $this->db->update('tblitems'); 
        redirect(admin_url('warehouse/commodity_list'));
    }
    ...
    public function get_good_receipt_row_template()
    {
		...
		echo $this->warehouse_model->create_goods_receipt_row_template([], $name, $commodity_name, $warehouse_id, $quantities, $unit_name, $unit_price, $taxname, $lot_number, $date_manufacture, $expiry_date, $commodity_code, $unit_id, $tax_rate, $tax_money, $goods_money, $note, $item_key,$feet,$Meter,$SKU,$Description,$discount);
	...
	public function get_good_delivery_row_template()
	{
		...
		echo $this->warehouse_model->create_goods_delivery_row_template([], $name, $commodity_name, $warehouse_id, $available_quantity, $quantities, $unit_name, $unit_price, $taxname, $commodity_code, $unit_id, $tax_rate, '', $discount, '', '', $guarantee_period, $expiry_date, $lot_number, $note, '', '', '', $item_key,$long_description,$sku_code,$feet,$meter);
	}
