...
function getVendorCurrency($skucode) {
    $CI = & get_instance();
    $vendor_currency = $CI->db->query("SELECT * FROM `tblpur_vendor` WHERE `userid` = '".$skucode."'")->row();
    $currency =  $vendor_currency->default_currency;
    
    $user_information = $CI->db->query("SELECT * FROM `tblcurrencies` WHERE `id` = '".$currency."'")->row();
    return  $user_information->symbol;
}
...
function getLongDescription($userid) {
    $CI = & get_instance();
    $user_information = $CI->db->query("SELECT * FROM `tblitems` WHERE id = '".$userid."'")->row();
    return $user_information->long_description;
}

function getFeetst($userid) {
    $CI = & get_instance();
    $user_information = $CI->db->query("SELECT * FROM `tblitems` WHERE id = '".$userid."'")->row();
    return $user_information->Feet;
}

function getMeterst($userid) {
    $CI = & get_instance();
    $user_information = $CI->db->query("SELECT * FROM `tblitems` WHERE id = '".$userid."'")->row();
    return $user_information->Meters;
}



function getTotalPurchaseQuantity($userid) {
    $CI = & get_instance();
    
    $user_informations = $CI->db->query("SELECT SUM(initail_quantity) as totalquantity FROM `tblitems` WHERE id = '".$userid."'")->row();
    
    $user_information = $CI->db->query("SELECT SUM(quantities) as totalquantity,SUM(goods_money) as total_goods_money FROM `tblgoods_receipt_detail` WHERE commodity_code = '".$userid."'")->row();
    return $user_information->totalquantity + $user_informations->totalquantity;
}

function getTotalPurchaseGoodMoney($userid) {
    $CI = & get_instance();
    $user_information = $CI->db->query("SELECT SUM(quantities) as totalquantity,SUM(goods_money) as total_goods_money FROM `tblgoods_receipt_detail` WHERE commodity_code = '".$userid."'")->row();
    return $user_information->total_goods_money;
}


function getTotalInvoiceQuantity($userid) {
    $CI = & get_instance();
    $user_information = $CI->db->query("SELECT SUM(qty) as totalquantity,SUM(rate) as total_goods_money FROM `tblitemable` WHERE sku_code = '".$userid."' AND rel_type = 'Invoice'")->row();
    return $user_information->totalquantity;
}

function getTotalRefundQuantity($userid) {
    $CI = & get_instance();
    $user_information = $CI->db->query("SELECT SUM(qty) as totalquantity,SUM(rate) as total_goods_money FROM `tblitemable` WHERE sku_code = '".$userid."' AND rel_type = 'credit_note'")->row();
    return $user_information->totalquantity;
}

function getTotalInvoiceGoodMoney($userid) {
    $CI = & get_instance();
    $user_information = $CI->db->query("SELECT SUM(qty) as totalquantity,SUM(rate) as total_goods_money FROM `tblitemable` WHERE sku_code = '".$userid."' AND rel_type = 'Invoice'")->row();
    return $user_information->total_goods_money;
}

function getTotalRefundGoodMoney($userid) {
    $CI = & get_instance();
    $user_information = $CI->db->query("SELECT SUM(qty) as totalquantity,SUM(rate) as total_goods_money FROM `tblitemable` WHERE sku_code = '".$userid."' AND rel_type = 'credit_note'")->row();
    return $user_information->total_goods_money;
}

function getTotalReturnQuantity($userid) {
    $CI = & get_instance();
    $user_information = $CI->db->query("SELECT SUM(quantities) as totalquantity,SUM(goods_money) as total_goods_money FROM `tblgoods_return_detail` WHERE commodity_code = '".$userid."'")->row();
    return $user_information->totalquantity;
}

function getTotalReturnGoodMoney($userid) {
    $CI = & get_instance();
    $user_information = $CI->db->query("SELECT SUM(quantities) as totalquantity,SUM(goods_money) as total_goods_money FROM `tblgoods_return_detail` WHERE commodity_code = '".$userid."'")->row();
    return $user_information->total_goods_money;
}
...
function get_subcategory_name($id = false)
{
    $CI           = & get_instance();
    ...
    if ($id == false) {
        return $CI->db->query('select * from tblwh_sub_group')->result_array();
    }

}


function get_category_name($id = false)
{
    $CI           = & get_instance();
