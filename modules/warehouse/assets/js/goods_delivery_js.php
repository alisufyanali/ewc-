...
function wh_add_item_to_preview(id) {
  ...
  requestGetJSON('warehouse/get_item_by_id/' + id +'/'+true).done(function (response) {
    ...
    $('.main input[name="sku_code"]').val(response.sku_code.replace(/(<|&lt;)br\s*\/*(>|&gt;)/g, " "));
    $('.main input[name="feet"]').val(response.Feet);
    $('.main input[name="meter"]').val(response.Meters);
...
//   alert('Here !');    
  ...
  data.unit_price = data.rate;
  console.log(data);
  
  
   var long_description = $('.main textarea[name="long_description"]').val();
   var sku_code =  $('.main input[name="sku_code"]').val();
   var feet =  $('.main input[name="feet"]').val();
   var meter =  $('.main input[name="meter"]').val();
  
  wh_get_item_row_template('newitems[' + item_key + ']',data.commodity_name,'1', '1', data.quantities, 'PCs','5600', '', '1001', data.expiry_date,'38', '1', '17', '0', '', '0' , '',long_description,sku_code,feet,meter).done(function(output){
  table_row += output;
...
function wh_get_item_row_template(name, commodity_name, warehouse_id, available_quantity, quantities, unit_name, unit_price, taxname, lot_number, expiry_date, commodity_code, unit_id, tax_rate, discount, note, guarantee_period, item_key,long_description,sku_code,feet,meter)  {
  "use strict";
