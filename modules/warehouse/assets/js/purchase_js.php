...
function wh_add_item_to_preview(id) {
	...
	requestGetJSON('warehouse/get_item_by_id/' + id).done(function (response) {
		...
		$('.main textarea[name="commodity_name"]').val(response.sub_group_name + ' ' + response.group_name + ' ' + response.Breadth + ' X ' + response.Width);
        ...
        $('#feet').val(response.Feet);
        $('#SKU').val(response.sku_code);
        $('#LongDescription').val(response.long_description);
        $('#meter').val(response.Meters);
        
		if($('select[name="warehouse_id_m"]').val() != ''){
			$('.main select[name="warehouse_id"]').val(18);
			init_selectpicker();
...
function wh_get_item_preview_values() {
	...
	response.warehouse_id = '18';
	response.quantities = $('.invoice-item .main input[name="quantities"]').val();
	
	response.feet = $('.invoice-item .main input[name="feet"]').val();
	response.meter = $('.invoice-item .main input[name="meter"]').val();
	response.SKU = $('.invoice-item .main input[name="SKU"]').val();
	response.Description = $('.invoice-item .main input[name="LongDescription"]').val();
...
function wh_get_item_row_template(name, commodity_name, warehouse_id, quantities, unit_name, unit_price, taxname, lot_number, date_manufacture, expiry_date, commodity_code, unit_id, tax_rate, tax_money, goods_money, note, item_key,feet,Meter,SKU,Description,discount)  {
	"use strict";
...
function wh_calculate_total(){
	...
	var calculated_tax,
		...
		taxes_rows = [],
		subtotal = 0,
		subtotals = $('input[name="subtotals"]').val() ? $('input[name="subtotals"]').val() : 0,
		total = 0,
		total_tax_money = 0,
		quantity = 1,
    ...
    $.each(rows, function () {
		...
		_amount = parseFloat(_amount) - parseFloat(discount);
	...
	$.each(taxes, function (taxname, total_tax) {
		...
		total_tax = (total_tax);
		$('#tax_id_' + slugify(taxname)).html(accounting.toFixed(total_tax));
	});
	...
	var discount_html = '-' + total_discount_calculated;
	...
	var ss = subtotal + hidden_input('total_goods_money', accounting.toFixed(subtotal, app.options.decimal_places)) + hidden_input('value_of_inventory', accounting.toFixed(subtotal, app.options.decimal_places));
		$('.wh-subtotal').html(currency + subtotal + hidden_input('total_goods_money', accounting.toFixed(subtotal, app.options.decimal_places)) + hidden_input('value_of_inventory', accounting.toFixed(subtotal, app.options.decimal_places)));
	

	$('.inventory_value').remove();
	var total_inventory_value = '<tr class="inventory_value"><td><span class="bold"><?php echo _l('value_of_inventory'); ?> :</span></td><td class="">' + (subtotal)+'</td></tr>';
	$('#subtotal').after(currency + total_inventory_value);

	$('.total_tax_value').remove();
	var total_tax_value = '<tr class="total_tax_value"><td><span class="bold"><?php echo _l('total_tax_money'); ?> :</span></td><td class="">' + (accounting.toFixed(total_tax_money))+'</td></tr>';
	$('#totalmoney').before(currency + total_tax_value);
    //var total = total + hidden_input('total_tax_money', accounting.toFixed(total_tax_money, app.options.decimal_places)) + hidden_input('total_money', accounting.toFixed(total, app.options.decimal_places));
	$('.wh-total').html(currency + accounting.toFixed((total), app.options.decimal_places) + hidden_input('total_tax_money', accounting.toFixed(total_tax_money, app.options.decimal_places)) + hidden_input('total_money', accounting.toFixed(total, app.options.decimal_places)));
	var rconversion = $('input[name="rconversion"]').val();
	...
	if(subtotals){
	    $('.wh-subtotals').html('PKR' + ' ' + accounting.toFixed((parseFloat(subtotals)), app.options.decimal_places) + hidden_input('subtotals', accounting.toFixed((parseFloat(total) * parseFloat(rconversion)), app.options.decimal_places)));
	}else{
        $('.wh-subtotals').html('PKR' + ' ' + accounting.toFixed((parseFloat(total) * parseFloat(rconversion)), app.options.decimal_places) + hidden_input('subtotals', accounting.toFixed((parseFloat(total) * parseFloat(rconversion)), app.options.decimal_places)));
	}
