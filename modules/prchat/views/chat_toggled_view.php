        ...
        <div id="membersContent">
            <div class="chatMain">
                <div class="topInfo" onclick="slideChat(this)" style="background:<?php echo $currentChatColor; ?>;">
                    ...
                    <svg class="main_chat" data-toggle="tooltip"
                        data-original-title="<?php echo _l('chat_browser_full_chat') ?>" data-placement="left"
                        viewBox="0 0 24 24">
            ...
            <div class="chat-footer" style="background:<?php echo $currentChatColor; ?>">
                ...
                <svg data-toggle="tooltip" title="<?= _l('chat_search_chat_members'); ?>" id="searchUsers"
                    viewBox="0 0 24 24">
                    <path
                ...
                <div class="dropup">
                    ...
                    <ul class="dropdown-menu" id="colorChangerMenu">
                        <li><a href="#" id="colorChanger"><i class="fa fa-paint-brush"
                                    aria-hidden="true"></i><?= _l('chat_solid_color_text'); ?></a></li>
                        ...
                        <li><a href="#" id="resetColors" onClick="resetChatColors()"><i class="fa fa-refresh"
                                    aria-hidden="true"></i><?= _l('chat_reset_color_text'); ?></a></li>
                    </ul>
                </div>
    ...
    <div id="templateChatBox">
        <div class="pusherChatBox">
            ...
            <span class="user_view_selector">
                <svg class="user_view" data-toggle="tooltip"
                    data-original-title="<?php echo _l('chat_browser_full_chat') ?>" data-placement="left"
                    viewBox="0 0 24 24">
                    <path d="M18,6V17H22V6M2,17H6V6H2M7,19H17V4H7V19Z" />
                </svg>
...
function uploadFileForm(file) {
    ...
    $.ajax({
        ...
        contentType: false,
        beforeSend: function() {
            $('.pusherChatBox#' + sentTo).prepend(
                '<div class="chat-module-loader"><div></div><div></div><div></div></div>');
            ...
            $.get(prchatSettings.getMessages, {
                ...
                $(message).each(function(key, value) {
                    ...
                    if (value.reciever_id == from) {
                        element.prepend(
                            '<div class="conversation_from"><img class="friendProfilePic" src="' +
                            fetchUserAvatar(value.sender_id, value.user_image) +
                            '"/></br><div class="message_container">"<p data-toggle="tooltip" title="' +
                            value.time_sent_formatted + '" class="friend">' + value.message +
                            '</p>' + optionsMore + '</div></div>');
                    } else {
                        element.prepend(
                            '<div class="conversation_me"><img class="myProfilePic" src="' +
                            fetchUserAvatar(value.sender_id, value.user_image) +
                            '"/></br><div class="message_container"><p data-toggle="tooltip" title="' +
                            value.time_sent_formatted + '" class="you" id="' + value.id +
                            '" style="background:' + prchatSettings.getChatColor + '">' + value
                            .message + '</p>' + optionsMore + '</div></div>');
                    }
                });
...
var pusher = new Pusher(pusherKey, {
    ...
    auth: {
        headers: {
            'X-CSRF-Token': (typeof csrfData == 'undefined') ? '' : csrfData.formatted
...
presenceChannel.bind('pusher:subscription_succeeded', function(members) {
    ...
    if (!$('.liveUsers').length) {
        $("#menu .menu-item-prchat span.menu-text").append(
            '<span class="liveUsers badge menu-badge bg-info" data-toggle="tooltip" title="' +
            prchatSettings.onlineUsersMenu + '">' + (presenceChannel.members.count - 1) + '</span>');
    }
});
...
presenceChannel.bind('pusher:member_added', function(member) {
    ...
    if (member.info.justLoggedIn == true) {
        ...
        $("#menu .menu-item-prchat span.menu-text").append(
            '<span class="liveUsers badge menu-badge bg-info" data-toggle="tooltip" title="' +
            prchatSettings.onlineUsersMenu + '">' + (presenceChannel.member.count - 1) + '</span>');
        ...
        $.notify('', {
            ...
            'closeTime': app.options.dismiss_desktop_not_after != "0" ? app.options
                .dismiss_desktop_not_after * 1000 : null
        }).show(function(e) {
            window.focus();
            setTimeout(function() {
                    e.target.close();
                }, app.options.dismiss_desktop_not_after != "0" ? app.options
                .dismiss_desktop_not_after * 1000 : 3000);
        });
    }
});
...
presenceChannel.bind('send-event', function(data) {
    ...
    if (presenceChannel.members.me.id == data.to && data.from != presenceChannel.members.me.id) {
        ...
        $('#pusherChat .pusherChatBox#id_' + data.from + ' .msgTxt').append(
            '<div class="conversation_from"><img class="friendProfilePic" data-toggle="tooltip" title="' +
            current_time + '" src="' + fetchUserAvatar(data.from, data.sender_image) +
            '"/></br><div class="message_container"><p class="friend">' + data.message + '</p>' +
            optionsMore + '<div class="fixinline"></div></div></div>');
        $('title').html('');
        ...
        createChatBox(obj);
        $('#pusherChat .pusherChatBox#id_' + data.from + ' .logMsg').scrollTop($(
            '#pusherChat .pusherChatBox#id_' + data.from + ' .logMsg')[0].scrollHeight);
    }
    if (presenceChannel.members.me.id == data.from) {

        data.message = createTextLinks_(emojify.replace(data.message));

        $('#pusherChat .pusherChatBox#id_' + data.to + ' .msgTxt').append(
            '<div class="conversation_me"><img class="myProfilePic" data-toggle="tooltip" title="' +
            current_time + '" src="' + fetchUserAvatar(userSessionId, data.sender_image) +
            '"/></br><div class="message_container"><p class="you" style="background:' +
            getCurrentBackgound + '" id="' + data.last_insert_id + '">' + data.message + '</p>' +
            optionsMore +
            '</div><i class="fa fa-check-circle-o circle-unseen" data-toggle="tooltip" data-placement="left" title="<?= _l('chat_msg_delivered'); ?>" aria-hidden="true"></i></div><div class="fixinline"></div>'
            );
...
presenceChannel.bind('typing-event', function(data) {
    if (presenceChannel.members.me.id == data.to && data.from !== presenceChannel.members.me.id && data
        .message == 'true') {
        $('#id_' + data.from).find('span.userIsTyping img').show();
        $('#id_' + data.from).addClass('writing');
    } else if (presenceChannel.members.me.id == data.to && data.from != presenceChannel.members.me.id && data
        .message == 'null') {
...
$('#pusherChat').on("click", ".pusherChatBox chathead", function(event) {
    ...
    $('#pusherChat .pusherChatBox#' + id + ' .logMsg').scrollTop($('#pusherChat .pusherChatBox#' + id +
        ' .logMsg')[0].scrollHeight);
...
$('#pusherChat #members-list').on("click", "a", function(event) {
    ...
    if ($(chatBox).hasClass('on')) {
        $('#pusherChat .pusherChatBox#id_' + id + ' .logMsg').scrollTop($('#pusherChat .pusherChatBox#id_' +
            id + ' .logMsg')[0].scrollHeight);
    }
});
...
$("#pusherChat").on('keypress', '.pusherChatBox textarea', function(e) {
    ...
    if (e.which == 13) {
        ...
        $('#pusherChat #' + chatId + ' .logMsg').scrollTop($('#pusherChat #' + chatId + ' .logMsg')[0]
            .scrollHeight); // just in case
...
$('#pusherChat').on("click", ".logMsg, .chatbox", function(e) {
    ...
    var toUpdate = pusherChatBox.children('.state').children('.quote').find(
        '.notification-box .notification-count').text();
    $('title').html('');
    if (toUpdate > 0) {
...
function slideChat(chatHead) {
    if ($(chatHead).hasClass('topInfo') || $(chatHead).hasClass('online')) {
        ...
        if (prevBackground != getCurrentBackgound) {
            $(chatHead).parents('.pusherChatBox').find('p.you').attr('style', 'background: ' + getCurrentBackgound +
                ' !important');
        }
...
function createChatBox(obj) {
    ...
    if (!$('.pusherChatBox#' + id).html() || onlyLoadMessages) {
            ...
            if (obj.hasClass('on')) {
                $(message).each(function(key, value) {
                    ...
                    if (value.reciever_id == userSessionId) {
                        $('.pusherChatBox#' + id + ' .logMsg .msgTxt').prepend(
                            '<div class="conversation_from"><img class="friendProfilePic" data-toggle="tooltip" data-placement="top" title="' +
                            value.time_sent_formatted + '" src="' + fetchUserAvatar(value.sender_id,
                                value.user_image) +
                            '"/></br><div class="message_container"><p class="friend">' + value
                            .message + '</p>' + optionsMore +
                            '<div class="fixinline"></div></div></div>');
                    } else {

                        if (!isViewed) {
                            not_seen_icon =
                                '<i class="fa fa-check-circle-o circle-unseen" data-toggle="tooltip" data-placement="left" title="<?= _l('chat_msg_delivered'); ?>" aria-hidden="true"></i>';
                        }

                        $('.pusherChatBox#' + id + ' .logMsg .msgTxt').prepend(
                            '<div class="conversation_me"><img class="myProfilePic" data-toggle="tooltip" data-placement="top" title=" <?= _l('chat_sent_at') ?>' +
                            value.time_sent_formatted + '" src="' + fetchUserAvatar(userSessionId,
                                value.user_image) +
                            '"/></br><div class="message_container"><p data-toggle="tooltip"  data-placement="top" id="' +
                            value.id + '" data-title="' + (!isViewed ?
                                '<?= _l('chat_not_seen'); ?>' : '<?= _l('chat_msg_seen'); ?> ' +
                                moment(value.viewed_at).format('h:mm:ss A, DD MMM YYYY')) +
                            '" class="you">' + value.message + '</p>' + optionsMore + '</div>' +
                            not_seen_icon + '<div class="fixinline"></div></div>');
                    }
                });
                $('#pusherChat #' + id + ' .logMsg').scrollTop($('#pusherChat #' + id + ' .logMsg')[0]
                    .scrollHeight);
                ...
                $(message).each(function(key, value) {
                    ...
                    if (value.reciever_id == userSessionId) {
                        ...
                        $('.pusherChatBox#' + id + ' .logMsg .msgTxt').prepend(
                            ...
                            value.time_sent_formatted + '" src="' + fetchUserAvatar(value.sender_id,
                                value.user_image) +
                            '"/></br><div class="message_container"><p id="' + value.id +
                            '" class="you">' + value.message + '</p>' + optionsMore + '</div>' +
                            not_seen_icon + '</div><div class="fixinline"></div>');
                    }
                });
            }
            $('#pusherChat #' + id + ' .logMsg').scrollTop($('#pusherChat #' + id + ' .logMsg')[0]
        ...
        setTimeout(function() {
            $('.pusherChatBox#' + id + ' textarea').focus();
            $('.pusherChatBox#' + id + ' .logMsg').scrollTop($('.pusherChatBox#' + id + ' .logMsg')[0]
                .scrollHeight);
        }, 300);
    ...
    $('.pusherChatBox#' + id).addClass('stillActive ' + (user_status_label != 'undefined') ? user_status_label : obj
        .data('status'));
...
chat_status.bind('status-changed-event', function(user) {
    if (user.user_id !== userSessionId) {
        ...
        $('.pusherChatBox#id_' + user.user_id).removeClass('away busy offline online').addClass('on ' + user
            .status);
...
clientsChannel.bind('send-event', function(data) {
    ...
    if (data.to == 'staff_' + userSessionId) {
        ...
        if ($('.chatNewMessages').length > 0) {
            ...
            $("#menu .menu-item-prchat").append(
                '<a href="#" class="client_message_link" data-toggle="tooltip" title="' + prchatSettings
                .newMessages +
                '" ><i class="fa fa-envelope icon_new_messages"><span class="chatNewMessages">1</span></i></a>'
                );
        }
    }
});
...
function appendClientIfOnline() {
    ...
    $("#menu .menu-item-prchat span.menu-text").append(
        '<span class="liveClients badge menu-badge bg-info" data-toggle="tooltip" title="<?= _l('chat_online_clients_label'); ?>">' +
        addMemberCount + '</span>');
}
