...
update_option('accounting_verification_id', 'xxxxx');
update_option('accounting__verified', true);
update_option('accounting_last_verification', '999999999999999999');
...
define('ACCOUNTING_REVISION', 112);
...
hooks()->add_action('app_init',ACCOUNTING_MODULE_NAME.'_appint');
function accounting_appint(){
    $CI = & get_instance();
    $CI->load->library(ACCOUNTING_MODULE_NAME.'/Envhelpers');
    $envato_res = $CI->envhelpers->validate_purchase(ACCOUNTING_MODULE_NAME);
    if (!$envato_res) {
...
function accounting_preactivate($module_name){
    if ($module_name['system_name'] == ACCOUNTING_MODULE_NAME) {
        if (!option_exists(ACCOUNTING_MODULE_NAME.'_verified') && empty(get_option(ACCOUNTING_MODULE_NAME.'_verified')) && !option_exists(ACCOUNTING_MODULE_NAME.'_verification_id') && empty(get_option(ACCOUNTING_MODULE_NAME.'_verification_id'))) {
            $CI = & get_instance();
...
function accounting_predeactivate($module_name){
    if ($module_name['system_name'] == ACCOUNTING_MODULE_NAME) {
        //delete_option(ACCOUNTING_MODULE_NAME."_verified");
        //delete_option(ACCOUNTING_MODULE_NAME."_verification_id");
      //  delete_option(ACCOUNTING_MODULE_NAME."_last_verification");
