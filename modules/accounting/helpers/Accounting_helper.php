...
function get_AllSupplierList() {
    $CI = & get_instance();
    $user_information = $CI->db->query("SELECT * FROM `tblpur_vendor`")->result_array();
    return $user_information;
}


function get_accountsList() {
    $CI = & get_instance();
    $user_information = $CI->db->query("SELECT * FROM `tblacc_accounts` WHERE account_type_id = '3' AND account_detail_type_id = '14'")->result_array();
    return $user_information;
}
...
function get_url_by_type_id($rel_type, $rel_id){
	...
	switch ($rel_type) {
        ...
        break;
        case 'received_entry':
            $url = admin_url('accounting/new_received_entry/'.$rel_id);
        break;
